...
top20_long <- top20 %>% select(orthogroup,iBAQ_SL,iBAQ_SA) %>% gather(species,iBAQ,-orthogroup) 

ggplot(top20_long,aes(x=orthogroup,y=iBAQ)) + 
  geom_bar(aes(x=orthogroup,y=iBAQ),stat="identity") + 
  facet_wrap(~species,ncol = 1) + 
  theme(axis.text.x= element_text(angle=90))

```

```{r manual editing, include = TRUE, warning= FALSE}

#this code is to manually edit the graph to colour based on manual groupings made based on swissprot identifiers

#have to write out the tables to csv files to edit 
write.csv(x = top20iBAQ_SL, file = "raw_data/SLtop20.csv")
write.csv(x = top20iBAQ_SA, file = "raw_data/SAtop20.csv")
#then read them back in after manually editing
top20edit_SL <- read.table("raw_data/SLtop20edit.csv",sep=',', header = TRUE)
top20edit_SA <- read.table("raw_data/SAtop20edit.csv",sep=',', header = TRUE)

#run the top20 and top20_long including description
top20edit <- rbind(top20edit_SA,top20edit_SL)
top20_longedit <- top20edit %>% select(orthogroup,iBAQ_SL,iBAQ_SA,Description) %>% gather(species,iBAQ,-orthogroup, -Description)
#order by Description colours 
top20_longedit$orthogroup <- factor(top20_longedit$orthogroup, levels = top20_longedit$orthogroup [order(top20_longedit$Description)])

#run graph using description as the fill
ggplot(top20_longedit,aes(x=orthogroup,y=iBAQ)) + 
  geom_bar(aes(x=orthogroup,y=iBAQ, fill=Description),stat="identity") + 
  facet_wrap(~species,ncol = 1) + scale_fill_manual(values = c('Actin' = "lightslateblue", 'Enzymatic' = "yellowgreen", 'Histone' = "deeppink", 'Calmodulin' = "darkorchid", 'Other' = "dodgerblue", 'Intermediate Filament' = 'darkorange','Tubulin' = "lightpink",'Annexin' = "darkblue")) +
  theme(axis.text.x= element_text(angle=90))
